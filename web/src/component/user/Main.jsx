...
class Main extends React.Component {
    componentWillMount = async () => {
        await this.props.findAllByGenreInAndStartTime({
            genreList: Array.from(Genre.keys()),
            startTime: Date.now()
        });
        await this.props.findAllPlayingNowInRegion(
            Object.keys(this.props.location)[0]
        );
        await this.props.findAllWillPlayInRegion(
            Object.keys(this.props.location)[0]
        );
    };
    ...
    onPlayingNowListClick = async () => {
        ...
        await this.props.route(
            RouteTable.CUSTOMER.ShowList.path + '#playingNow',
            this.props.isAuthed,
            this.props.role
        );
    };
    ...
    onWillPlayListClick = async () => {
        ...
        await this.props.route(
            RouteTable.CUSTOMER.ShowList.path + '#willPlay',
            this.props.isAuthed,
            this.props.role
        );
    };
    onShowSelect = async s => {
        ...
        await this.props.route(
            RouteTable.CUSTOMER.ShowInfo.path,
            this.props.isAuthed
        );
    };
    render = () => {
        const settings = {
            ...
            centerMode: true,
            infinite: true,
            initialSlide: this.props.showsPlayingNowInRegion.length - 1,
            centerPadding: '60px',
            slidesToShow: 5,
            slidesToScroll: 1,
            swipeToSlide: false,
        ...
        switch (this.props.role) {
            case Role.CUSTOMER:
                return (
                    <div>
                        <Slider {...settings}>
                            {this.props.showsPlayingNowInRegion
                                .slice(0, 6)
                                .map((s, index) => (
                                    <img
                                        alt={s.name}
                                        key={index}
                                        onClick={() => this.onShowSelect(s)}
                                        style={{ width: '160px' }}
                                        src={s.poster}
                                    />
                                ))}
                        ...
                        <div className="two-shows-panel">
                            <div align="left" className="shows">
                                ...
                                >
                                    ...
                                    <Col span={6} offset={12} align="right">
                                        <Button
                                            onClick={this.onPlayingNowListClick}
                                        >
                                            全部<Icon type="right" />
                                        </Button>
                                    </Col>
                            ...
                            <div align="left" className="shows">
                                ...
                                >
                                    <Col span={6}>
                                        <h1>
                                            即将上映（{
                                                this.props.showsWillPlayInRegion
                                                    .length
                                            }部）
                                        </h1>
                                    </Col>
                                    <Col span={6} offset={12} align="right">
                                        <Button
                                            onClick={this.onWillPlayListClick}
                                        >
                                            全部<Icon type="right" />
                                        </Button>
                                    </Col>
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        findAllPlayingNowInRegion: regionId =>
            dispatch(findAllPlayingNowInRegion(regionId)),
        findAllWillPlayInRegion: regionId =>
            dispatch(findAllWillPlayInRegion(regionId)),

        findAllByGenreInAndStartTime: elems =>
            dispatch(findAllByGenreInAndStartTime(elems)),
